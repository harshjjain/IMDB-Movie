...
package group14final.imdb_title_crew_stg_0_1;
...
/**
 * Job: imdb_title_crew_stg Purpose: <br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class imdb_title_crew_stg implements TalendJob {
	static {
		System.setProperty("TalendJob.log", "imdb_title_crew_stg.log");
	}

	private static org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager
			.getLogger(imdb_title_crew_stg.class);
	...
	private final String jobName = "imdb_title_crew_stg";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					imdb_title_crew_stg.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(imdb_title_crew_stg.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class mysqlStruct implements routines.system.IPersistableRow<mysqlStruct> {
		final static byte[] commonByteArrayLock_GROUP14FINAL_imdb_title_crew_stg = new byte[0];
		static byte[] commonByteArray_GROUP14FINAL_imdb_title_crew_stg = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_GROUP14FINAL_imdb_title_crew_stg.length) {
					if (length < 1024 && commonByteArray_GROUP14FINAL_imdb_title_crew_stg.length == 0) {
						commonByteArray_GROUP14FINAL_imdb_title_crew_stg = new byte[1024];
					} else {
						commonByteArray_GROUP14FINAL_imdb_title_crew_stg = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_GROUP14FINAL_imdb_title_crew_stg, 0, length);
				strReturn = new String(commonByteArray_GROUP14FINAL_imdb_title_crew_stg, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_GROUP14FINAL_imdb_title_crew_stg, 0, length);
				strReturn = new String(commonByteArray_GROUP14FINAL_imdb_title_crew_stg, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_GROUP14FINAL_imdb_title_crew_stg) {
	...
	public void tDBInput_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		org.slf4j.MDC.put("_subJobPid", "17vXAj_" + subJobPidCounter.getAndIncrement());
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
					...
					class BytesLimit65535_tDBOutput_1 {
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tDBOutput_1.append("PASS" + " = "
									...
									+ "...");
							log4jParamters_tDBOutput_1.append(" | ");
							log4jParamters_tDBOutput_1.append("TABLE" + " = " + "\"imdb_title_crew_stg\"");
							log4jParamters_tDBOutput_1.append(" | ");
				...
				String tableName_tDBOutput_1 = "imdb_title_crew_stg";
				...
				while (rsTable_tDBOutput_1.next()) {
					...
					if (table_tDBOutput_1.equalsIgnoreCase("imdb_title_crew_stg")) {
				...
				if (!whetherExist_tDBOutput_1) {
					try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
						...
						stmtCreate_tDBOutput_1.execute("CREATE TABLE `" + tableName_tDBOutput_1
								+ "`(`tconst` VARCHAR(10)   not null ,`directors` VARCHAR(4000)  ,`writers` VARCHAR(4000)  ,`DI_Create_DT` DATETIME default CURRENT_TIMESTAMP  not null ,primary key(`tconst`))");
						if (log.isDebugEnabled())
				...
				String insert_tDBOutput_1 = "INSERT INTO `" + "imdb_title_crew_stg"
						+ "` (`tconst`,`directors`,`writers`,`DI_Create_DT`) VALUES (?,?,?,?)";
					...
					class BytesLimit65535_tDBInput_1 {
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tDBInput_1.append("QUERY" + " = "
									+ "\"SELECT    `imdb_title_crew`.`tconst`,    `imdb_title_crew`.`directors`,    `imdb_title_crew`.`writers`,    `imdb_title_crew`.`DI_Create_DT` FROM `imdb_title_crew`\"");
							log4jParamters_tDBInput_1.append(" | ");
							...
							log4jParamters_tDBInput_1.append("TRIM_COLUMN" + " = " + "[{TRIM=" + ("false")
									...
									+ ("directors") + "}, {TRIM=" + ("false") + ", SCHEMA_COLUMN=" + ("writers")
				...
				try {
					...
					while (rs_tDBInput_1.next()) {
						...
						if (runStat.update(execStat, enableLogStash, iterateId, 1, 1
								...
								, "row1", "tDBInput_1", "\"imdb_title_crew\"", "tMysqlInput", "tMap_1", "tMap_1", "tMap"
...
// # Output table : 'mysql'
							...
							mysql_tmp.directors = row1.directors;
							mysql_tmp.writers = row1.writers;
...
// Start of branch "mysql"
						if (mysql != null) {
							...
							if (mysql.directors == null) {
								pstmt_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
							} else {
								pstmt_tDBOutput_1.setString(2, mysql.directors);
							}

							if (mysql.writers == null) {
								pstmt_tDBOutput_1.setNull(3, java.sql.Types.VARCHAR);
							} else {
								pstmt_tDBOutput_1.setString(3, mysql.writers);
							}
...
// ###############################      
				...
				if (runStat.updateStatAndLog(execStat, enableLogStash, resourceMap, iterateId, "row1", 2, 0,
						"tDBInput_1", "\"imdb_title_crew\"", "tMysqlInput", "tMap_1", "tMap_1", "tMap", "output")) {
					talendJobLogProcess(globalMap);
				}
	...
	public void talendJobLogProcess(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		org.slf4j.MDC.put("_subJobPid", "2KIqPA_" + subJobPidCounter.getAndIncrement());
	...
	public static void main(String[] args) {
		final imdb_title_crew_stg imdb_title_crew_stgClass = new imdb_title_crew_stg();

		int exitCode = imdb_title_crew_stgClass.runJobInTOS(args);
		if (exitCode == 0) {
			log.info("TalendJob: 'imdb_title_crew_stg' - Done.");
		}
	...
	public int runJobInTOS(String[] args) {
		...
		getjobInfo();
		log.info("TalendJob: 'imdb_title_crew_stg' - Start.");
		...
		org.slf4j.MDC.put("_jobRepositoryId", "_vOMiUI_HEe62yM1cY-rR4Q");
		org.slf4j.MDC.put("_compiledAtTimestamp", "2023-11-30T22:39:03.611983800Z");
		...
		try {
			...
			java.io.InputStream inContext = imdb_title_crew_stg.class.getClassLoader()
					.getResourceAsStream("group14final/imdb_title_crew_stg_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = imdb_title_crew_stg.class.getClassLoader()
		...
		log.info("TalendJob: 'imdb_title_crew_stg' - Started.");
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : imdb_title_crew_stg");
		}
		if (enableLogStash) {
		...
		log.info("TalendJob: 'imdb_title_crew_stg' - Finished - status: " + status + " returnCode: " + returnCode);
...
/************************************************************************************************
 * 115092 characters generated by Talend Cloud Data Fabric on the November 30,
 * 2023 at 5:39:03 PM EST
 ************************************************************************************************/